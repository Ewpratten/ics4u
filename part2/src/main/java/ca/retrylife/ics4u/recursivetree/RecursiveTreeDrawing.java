package ca.retrylife.ics4u.recursivetree;
...
class RecursiveTreeDrawingPanel extends JPanel {
    ...
    private void drawTree(Graphics2D g, Point2D start, double length, double minLength, double width, double alpha) {
        ...
        Point2D end = new Point2D.Double(start.getX() + Math.sin(alpha + Math.PI) * length,
                start.getY() + Math.cos(alpha + Math.PI) * length);
        g.draw(new Line2D.Double(start, end));
        drawTree(g, end, length * leftLengthFactor, minLength, width * leftWidthFactor, alpha + leftAngleDelta);
        drawTree(g, end, length * rightLengthFactor, minLength, width * rightWidthFactor, alpha + rightAngleDelta);
    }
